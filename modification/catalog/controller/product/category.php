...
class ControllerProductCategory extends Controller {
	public function index() {
		...
		if ($category_info) {
			...
			$data['text_supplied'] = $this->language->get('text_supplied');
			...
			foreach ($results as $result) {
                ...
                if (strpos($this->config->get('config_template'), 'journal2') === 0 && $special && $this->journal2->settings->get('show_countdown', 'never') !== 'never') {
                    ...
                    $product_info_ = $this->model_catalog_product->getScreenname($result['product_id']);

		            if ($product_info_) {		
             	      $seller_name = $product_info_['screenname'];
                ...
                'labels'        => $this->model_journal2_product->getLabels($result['product_id']),
					...
					'description2' => utf8_substr(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8')), 0, $this->config->get('config_product_description_length')) . '..',
