...
class ModelAccountCustomer extends Model {
	public function addCustomer($data) {
		...
		$length = 50; // Panjang string yang diinginkan
		...
		$date = date('YmdHis');
		$codeaktivasi = $date . $randomString;
		...
		if (!$customer_group_info['approval']) {
			$message .= $this->language->get('text_login_tes') . "\n";
		} else {
			$message .= $this->language->get('text_approval') . "\n";
		}

		$message .= $this->url->link('account/aktivasi&code='. $codeaktivasi .'', '', 'SSL') . "\n\n";
		$message .= $this->language->get('text_services_tes') . "\n\n";
	...
	public function getDetailCustomer($customer_id){
		$query = $this->db->query("SELECT oc_customer.*,
	...
	public function getCartDetailCustomer($address_id){
	...
	public function getCartDetailSeller($address_id){
		$query = $this->db->query("SELECT oc_address.*,
		oc_zone.name
		FROM oc_address
		LEFT JOIN oc_zone ON oc_address.zone_id = oc_zone.zone_id
	...
	public function getIdSellerFromCart($customer_id){
		$query =  $this->db->query("SELECT * FROM oc_cart WHERE customer_id='$customer_id' ");
		return $query->row;
	}

	public function getStatusProductFromCart($customer_id){
		$query =  $this->db->query("SELECT IF(SUM(ukuran_unit) > 0, 1, 0) AS hasil FROM oc_cart WHERE customer_id='$customer_id' ");
		return $query->row;
	}

	public function getTotalFromCart($customer_id){
		$query = $this->db->query("SELECT 
		oc_cart.product_id, 
		oc_cart.quantity,
		oc_product.price,
		(oc_cart.quantity * oc_product.price) AS total_price,
		SUM(oc_cart.quantity * oc_product.price) AS total_keseluruhan
		FROM oc_cart
	...
	public function getTotalFromCartOption($customer_id){
		$query = $this->db->query("SELECT cart_id,quantity,`option` FROM oc_cart WHERE customer_id='$customer_id' AND `option` NOT LIKE '[]' ");
		return $query->rows;
	}

	public function getTotalCartNoOption($customer_id){
		$query = $this->db->query("SELECT 
		oc_cart.product_id, 
		oc_cart.quantity,
		oc_product.price,
		...
		FROM oc_cart
		LEFT JOIN oc_product ON oc_cart.product_id = oc_product.product_id
		WHERE oc_cart.customer_id='$customer_id' AND `option`='[]'
		");
	...
	public function editDataToCart($data){
		$customer_id = $data['customer_id'];
		$pilihan_tukang = $data['optiontukang'];
		$seller_address = $data['seller_address'];
		$customer_address = $data['customer_address'];

		$this->db->query("UPDATE oc_cart SET approve='1', origin='$seller_address', destination='$customer_address', optiontukang='$pilihan_tukang' WHERE customer_id='$customer_id' ");

	}

	public function restartDataToCart($data){
		$customer_id = $data['customer_id'];

		$this->db->query("UPDATE oc_cart SET kurir='', ongkir='', approve=0, klik=0, origin=0, destination=0, optiontukang=0, hargaongkir=0, hargatukang=0, batascheckout='0000-00-00 00:00:00' WHERE customer_id='$customer_id'");
	}
