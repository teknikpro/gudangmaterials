...
class ModelCustomerpartnerTransaction extends Model {
	...
	public function updateongkir($idtransaksi){

		$mysql = $this->db->query("SELECT SUM(total) AS jumlah FROM " . DB_PREFIX . "order WHERE id_transaksi = '".$idtransaksi."' ");
		$jumlah = $mysql->row['jumlah'];

		$query = $this->db->query("SELECT SUM(total) AS total FROM " . DB_PREFIX . "order_product WHERE id_transaksi = '".$idtransaksi."' ");
		$total =  $query->row['total'];

		$totalongkir = $jumlah - $total;

		$this->db->query("UPDATE ".DB_PREFIX."customerpartner_to_transaction SET update_ongkir = 1, ongkir = '".$totalongkir."'  WHERE id = '".$idtransaksi."' ");

		$this->db->query("UPDATE ".DB_PREFIX."order_product SET paid_status = 1 WHERE paid_status = 2 ");

		$this->db->query("UPDATE ".DB_PREFIX."order SET status_pembayaran = 2 WHERE id_transaksi = '".$idtransaksi."' ");
	...
	public function addTransaction($data) {
		...
		$id_transaksi = $this->db->query("SELECT * FROM `" . DB_PREFIX . "customerpartner_to_transaction` ORDER BY id DESC LIMIT 1 ")->row;
		$idtransaction = $id_transaksi['id'] + 1;
		...
		foreach ($data['select'] as $key => $value) {
			...
			foreach ($value as $key => $detail) {
				...
				$this->db->query("UPDATE ".DB_PREFIX."order_product SET paid_status = 2, id_transaksi = '".$idtransaction."' WHERE order_product_id = '".$detail."' ");
			}
			$this->db->query("UPDATE ".DB_PREFIX."order SET id_transaksi = '".$idtransaction."' WHERE order_id = '".$order_id."' ");
		}
		...
		if($this->config->get('wk_seller_group_status')) {
			...
			if (isset($commission['commission_amount']) && $commission['commission_amount']) {
				...
				if ($data['amount'] < 0) {
					$previous_commission['commission_paid'] += (float)$commission['commission_amount'] + (float)$data['amount'] + 1000;
					...
					$previous_commission['commission_paid'] += (float)$commission['commission_amount'] + 1000 ;
