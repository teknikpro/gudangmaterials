...
class ControllerCatalogProduct extends Controller {
	...
	public function add() {
        ...
        if ($this->config->get('marketplace_status') && isset($this->request->get['mpcheck'])) {
	...
	protected function getForm() {
        ...
        if ($this->config->get('marketplace_status') && isset($this->request->get['mpcheck'])) {
          if (!isset($this->request->get['product_id'])) {
            $data['action'] = $this->url->link('catalog/product/add', 'token=' . $this->session->data['token'] . $url . '&mpcheck=1', 'SSL');
          } else {
            $data['action'] = $this->url->link('catalog/product/edit', 'token=' . $this->session->data['token'] . '&mpcheck=1&product_id=' . $this->request->get['product_id'] . $url, 'SSL');
          }
          $data['cancel'] = $this->url->link('customerpartner/product', 'token=' . $this->session->data['token'] . $url, 'SSL');
        } else {
          if (!isset($this->request->get['product_id'])) {
            $data['action'] = $this->url->link('catalog/product/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
          } else {
        ...
        if($this->config->get('marketplace_status') && isset($this->request->get['product_id'])){
