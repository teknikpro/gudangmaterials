...
/**
	...
	public function getLinkLacak($order_id){
		$query = $this->db->query("SELECT linklacak FROM oc_order WHERE order_id='$order_id' ");
	...
	public function addOrderHistory($order_id,$data,$seller_change_order_status_name = '') {
		...
		$queryorder = $this->db->query("SELECT * FROM oc_order WHERE order_id='$order_id' ");
		$dataorder = $queryorder->row;
		$datatracking = $dataorder['tracking'];
		...
		if($datatracking){
			...
			if($data['order_status_id'] == 5){
				...
				$allowedProductIds = [614, 883, 334];

				$totalKomisi = 0;

				foreach ($datakomisi as $data) {
					if (in_array($data['product_id'], $allowedProductIds)) {
						// Hitung komisi
						$tarifKomisi = (float)$data['tarif_komisi'];
						$price = (float)$data['price'];
						$quantity = (int)$data['quantity'];
						
						$komisi = (($tarifKomisi / 100) * $price) * $quantity;
