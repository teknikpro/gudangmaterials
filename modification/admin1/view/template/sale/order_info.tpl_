...
<li><a href="#tab-shipping" data-toggle="tab"><?php echo $tab_shipment; ?></a></li>
        ...
        <div class="tab-content">
          <div class="tab-pane active" id="tab-history">
            ...
            <?php foreach ($order_statuses as $order_statuses) {
                        $style = isset($hp_tracking_sort_order[$order_statuses['order_status_id']]) ? "background-color: #8fbb6c; color: #FFF;" : ""; ?>
                        ?>
                      <?php if ($order_statuses['order_status_id'] == $order_status_id) { ?>
                      <option style="<?php echo $style; ?>" value="<?php echo $order_statuses['order_status_id']; ?>" selected="selected"><?php echo $order_statuses['name']; ?></option>
                      <?php } else { ?>
                      <option style="<?php echo $style; ?>" value="<?php echo $order_statuses['order_status_id']; ?>"><?php echo $order_statuses['name']; ?></option>
                      <?php } ?>
                      <?php } ?> 
                    </select>
                  </div>
          ...
          <div class="tab-pane" id="tab-shipping">
            ...
            <div id="shipping-number-form">
              ...
              <div class='alert alert-success'><i class='fa fa-check-circle'></i> Success</div>
                ...
                <form class="form-horizontal">
                  <div class="form-group">
                    <label class="col-sm-2 control-label" for="input-shipping-receipt"><?php echo $text_entry_shipping; ?></label>
                    <div class="col-sm-10">
                      <input type="text" name="shipping_number" id="shipping_number" value="<?php echo $shipping_number; ?>" placeholder="Input Your Shipping Receipt" id="input-shipping-receipt-id" class="form-control" />
                    </div>
                  </div>
                  <div class="form-group">
                    <label class="col-sm-2 control-label" for="input-order-status"><?php echo $entry_shipping_method; ?></label>
                    <div class="col-sm-10">
                      <select name="courier_name" id="courier" class="form-control">
                          <option value=""><?php echo $text_select; ?></option>
                        <?php foreach($shippings as $shipping) { ?>
                          <?php if($shipping['code'] == $courier) { ?>
                          <option value="<?php echo $shipping['code']; ?>" selected="selected"><?php echo $shipping['name']; ?></option>
                          <?php } else { ?>
                ...
                <?php $display = $shipping_number ? 'inline-block;' : 'none;'; ?>
                        <button style="display: <?php echo $display; ?>" id="button-shippingnumber-remove" data-loading-text="<?php echo $text_loading; ?>" class="btn btn-default"><i class="fa fa-trash"></i> <?php echo $button_shipping_remove; ?></button>

                  <button id="button-shippingnumber-add" data-loading-text="<?php echo $text_loading; ?>" class="btn btn-primary"><i class="fa fa-plus-circle"></i> <?php echo $text_shipping; ?></button>
                </div>
            </fieldset>
            </div>
          </div>
...
<script type="text/javascript">
// hpwd
$('#shipping-manifest').load('index.php?route=sale/order/shippingmanifest&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>');

$('#button-shippingnumber-add').on('click', function() {
	$.ajax({
		url: 'index.php?route=sale/order/addshippingnumber&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>',
		type: 'post',
...
$('#button-shippingnumber-remove').on('click', function() {
	$.ajax({
		url: 'index.php?route=sale/order/removeshippingnumber&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>',
		type: 'post',
		data: 'shipping_number=' + $('#shipping_number').val() + '&courier_name=' + $('#courier').val(),
		dataType: 'json',		
		beforeSend: function() {
