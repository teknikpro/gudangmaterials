...
class ControllerPaymentSnap extends Controller {
  ...
  public function process_order() {
    ...
    //$this->url->link('payment/snap/GetPaymentMethode', '', 'SSL');
    	 
	 

    $this->cart->UpdatePaymentMethode($this->session->data['order_id'],  $payment_methodX);
          ...
          switch ($payment_type) {
            case "bank_transfer":
              ...
              if(isset($response->va_numbers[0]->bank)){
                ...
                $payment_methodX = strtoupper($bank) . " Virtual Account";
				...
				$payment_methodX = "Permata Virtual Account";
              }
              else{
            ...
            case "echannel":
				...
				$payment_methodX = "Mandiri Bill Payment";
            ...
            case "cstore":
                ...
                $data['data']= array(
                'payment_type' => $payment_type,
				 'payment_method' => "Pembayaran di Konter",  
                 'instruction'      => $response->pdf_url,
				//'instruction'      => $this->url->link('account/order/info', 'order_id=' . $response->order_id, 'SSL'),
                'payment_code' => $response->payment_code
                );   
				
                $payment_methodX = "Pembayaran di Konter";
            ...
            default:
			  ...
			  $payment_methodX = "Pembayaran via gopay";
              break;
            }

		  
          $this->cart->clear();
		  $this->cart->UpdatePaymentMethode($response->order_id,  $payment_methodX);
		  
		   
		 /* $this->load->model('checkout/order');$this->url->link('payment/snap/process_order');
  ...
  public function payment_notification() {
    ...
    if($payment_type == "bank_transfer"){
      if($notif->va_numbers[0]->bank){
        $virtualnumber = $notif->va_numbers[0]->va_number;
      }else if($notif->permata_va_number){
        $virtualnumber = $notif->permata_va_number;
      }else {
        $virtualnumber = $notif->va_number;
      }
    }else {
      $virtualnumber = "";
    }
    ...
    if ($transaction == 'capture') {
      ...
      if ($fraud == 'challenge') {
        ...
        $this->model_checkout_order->addOrderHistory(
            $notif->order_id,
            5,
            $order_note . 'Payment status challenged. Please take action on ' . 'your Merchant Administration Portal. - ' . $payment_type,
            $virtualnumber
          );
      }
        ...
        $this->model_checkout_order->addOrderHistory(
            $notif->order_id,
            5,
            $order_note . 'Payment Completed - ' . $payment_type,
            $virtualnumber
          );
      }
    }
        ...
        $this->model_checkout_order->addOrderHistory(
            $notif->order_id,
            7,
            $order_note . 'Canceled Payment - ' . $payment_type,
            $virtualnumber
          );
    }
      ...
      $this->model_checkout_order->addOrderHistory(
          $notif->order_id,
          1,
          $order_note . 'Awaiting Payment - ' . $payment_type,
          $virtualnumber
        );
    }
      ...
      $this->model_checkout_order->addOrderHistory(
          $notif->order_id,
          14,
          $order_note . 'Expired Payment - ' . $payment_type,
          $virtualnumber
        );
    }
          ...
          if($payment_type != 'credit_card'){
              ...
              $virtualnumber
               ...
               echo "<script>localStorage.setItem('message','Sudah Bayar')</script>";
