...
class ControllerCommonFileManager extends Controller {
	public function index() {
		...
		foreach ($images as $image) {
			...
			if (is_dir($image)) {
				...
				$data['images'][] = array(
					...
					'href'  => isset($this->request->get['wk_ckeditor']) ? $this->url->link('common/filemanager', '&wk_ckeditor&CKEditorFuncNum=1&langCode=en&directory=' . urlencode(utf8_substr($image, utf8_strlen(DIR_IMAGE . 'wkseller/'.$dir.'/'))) . $url, 'SSL') : $this->url->link('common/filemanager', '&directory=' . urlencode(utf8_substr($image, utf8_strlen(DIR_IMAGE . 'wkseller/'.$dir.'/'))) . $url, 'SSL')
				);
		...
		$data['parent'] = $this->url->link('common/filemanager', ltrim($url, '&'), 'SSL');
		...
		$pagination->limit = 16;
		$pagination->url = $this->url->link('common/filemanager', ltrim($url, '&') . '&page={page}', 'SSL');
		...
		if (isset($this->request->get['wk_ckeditor'])) {
			...
			if ($this->config->get('config_template') == 'journal2') {
			    $this->response->setOutput($this->load->view('journal2/template/common/filemanager_j2.tpl', $data));
			}else{
				$this->response->setOutput($this->load->view('default/template/common/filemanager.tpl', $data));
			}
		}
