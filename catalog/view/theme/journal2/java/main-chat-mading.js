...
function getStateOfChat() {
  if (!instanse) {
    ...
    $.ajax({
      ...
      cache: false,
      url:
        "https://gudangmaterials.id/api4/consultant/index.php/member/chat-mading/" +
        userid +
        "/" +
        localStorage.userid +
        "//" +
  ...
  if (!instanse) {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data["status"] == "OK") {
          $("#like" + id).html(
            "<i class='fal fa-heart' style='margin-right:5px;color:red'></i> " +
              data.status +
              " Suka"
          );
        } else {
          $("#like" + id).html(
            "<i class='fal fa-heart' style='margin-right:5px;'></i> " +
              data.status +
              " Suka"
          );
        }
  ...
  if ($.trim(chatid).length > 0) {
    if (isimage_ == "1") {
      ...
      ft.upload(
        imageURI,
        encodeURI(
          "https://gudangmaterials.id/api4/consultant/index.php/member/chat-media-mading/" +
            localStorage.userid +
            "/?callback=?"
        ),
        win,
