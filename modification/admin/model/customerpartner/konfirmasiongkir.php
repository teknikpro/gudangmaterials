<?php
class ModelCustomerpartnerkonfirmasiOngkir extends Model {
	...
	public function getKonfirmasiData(){
		$query = $this->db->query("SELECT oc_cart.*, oc_customer.firstname, oc_customer.lastname FROM oc_cart LEFT JOIN oc_customer ON oc_cart.customer_id = oc_customer.customer_id WHERE approve>= 1 GROUP BY customer_id ORDER BY date_added DESC ");

		return $query->rows;
	}

	public function getDetailKonfirmasi($customer_id){
		$detailkonfir_query = $this->db->query("SELECT oc_cart.*,
		 oc_product_description.name, 
		 oc_product.model, 
		 oc_customerpartner_to_product.price,
		 oc_product_option.product_option_id
		 FROM oc_cart 
		 LEFT JOIN oc_product_description ON oc_cart.product_id = oc_product_description.product_id 
		 LEFT JOIN oc_product ON oc_cart.product_id = oc_product.product_id 
		 LEFT JOIN oc_customerpartner_to_product ON oc_cart.product_id = oc_customerpartner_to_product.product_id
		 LEFT JOIN oc_product_option ON oc_cart.product_id = oc_product_option.product_id
		 WHERE oc_cart.customer_id=$customer_id  ");

		return $detailkonfir_query->rows;
	}

	public function getHargaOption($idoptionvalues){
		$query = $this->db->query("SELECT price FROM oc_product_option_value WHERE product_option_value_id = $idoptionvalues");
		return $query->row;
	}

	public function getHargaSpesial($product_id){
		$today = date('Y-m-d');
		$query = $this->db->query("SELECT price FROM oc_product_special WHERE product_id = $product_id AND date_end >= '$today' ");
		return $query->row;
	}

	public function getIdSellerOrder($idseller){
		$query = $this->db->query("SELECT customer_id FROM oc_customerpartner_to_order WHERE order_id='$idseller' ");
		return $query->row;
	}

	public function getIdCustomerOrder($orderid){
		$query = $this->db->query("SELECT customer_id FROM oc_order WHERE order_id='$orderid' ");
		return $query->row;
	}

	public function getAddressIdSeller($customer_id){
		$query = $this->db->query("SELECT firstname, lastname, address_id FROM oc_customer WHERE customer_id='$customer_id' ");
		return $query->row;
	}

	public function getDataPayment($order_id){
		$query = $this->db->query("SELECT payment_method, payment_code, virtual_number FROM oc_order WHERE order_id='$order_id' ");
		return $query->row;
	}

	public function getAddressIdFromCart($cart_id){
		$query = $this->db->query("SELECT origin,destination FROM oc_cart WHERE cart_id='$cart_id' ");

		return $query->row;
	}

	public function getDataCustomer($address_id){
		$query = $this->db->query("SELECT oc_address.*,
		oc_zone.name
		FROM oc_address
		LEFT JOIN oc_zone ON oc_address.zone_id = oc_zone.zone_id
		WHERE oc_address.address_id=$address_id
		");

		return $query->row;
	}

	public function getDataSeller($address_id){
		$query = $this->db->query("SELECT oc_address.*,
	...
	public function getDataOngkir($cart_id){
		$query = $this->db->query("SELECT kurir, ongkir, optiontukang, hargaongkir, hargatukang FROM oc_cart WHERE cart_id='$cart_id' ");
		return $query->row;
	}
	...
	public function editOngkosKirim($data){
		...
		$hargaongkir = $data['harga'];
		$hargatukang = $data['hargatukang'];
		$harga = ($hargaongkir + $hargatukang);

		// waktu kurang 940
		$batascheckout = date("Y-m-d H:i:s", strtotime("+430 minutes"));
		
		$query = $this->db->query("UPDATE oc_cart SET kurir='$ekspedisi', ongkir='$harga', approve=2, hargaongkir='$hargaongkir', hargatukang='$hargatukang', batascheckout='$batascheckout' WHERE customer_id='$customer_id' ");
	}

	public function editLinkLacak($data){
		$order_id = $data['order_id'];
		$seller_id = $data['seller_id'];
		$link = $data['linklacak'];

		$query = $this->db->query("UPDATE oc_order SET linklacak='$link' WHERE order_id='$order_id' ");
