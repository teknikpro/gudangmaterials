...
<?php else: ?>
 ...
 <?php
  ...
  var baseurl = "<?php echo $base;?>";
    ...
    $.ajax({
      url: 'index.php?route=payment/snapinst/process_order',
      cache: false,
      beforeSend: function() {
  ...
  ?>
        ...
        <?php if ($disable_mixpanel == 0){?>
          trackResult(data, merch_id, 'oc2_installment_online', 'pay', null);
        <?php } ?>
        ...
        snap.pay(data, {
          ...
          onSuccess: function(result){
            <?php if ($disable_mixpanel == 0){?>
              trackResult(data, merch_id, 'oc2_installment_online', 'success', result);
            <?php } ?>
            changeResult('success', result, 'snapinst');
            console.log(result.status_message);
            $("#payment-form").submit();
          },
          onPending: function(result){
            <?php if ($disable_mixpanel == 0){?>
              trackResult(data, merch_id, 'oc2_installment_online', 'pending', result);
            <?php } ?>
            changeResult('pending', result, 'snapinst');
            console.log(result.status_message);
            $("#payment-form").submit();
          },
          onError: function(result){
            <?php if ($disable_mixpanel == 0){?>
              trackResult(data, merch_id, 'oc2_installment_online', 'error', result);
            <?php } ?>
            changeResult('error', result, 'snapinst');
            console.log(result.status_message);
            $.ajax({
          ...
          onClose: function(){
            <?php if ($disable_mixpanel == 0){?>
              trackResult(data, merch_id, 'oc2_installment_online', 'close');
            <?php } ?>
