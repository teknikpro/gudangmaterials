...
class ControllerJournal2Settings extends Controller {
    ...
    public function index() {
        ...
        if ($cache === null || self::$CACHEABLE !== true) {
			...
			// gdpr contact form
			if ($this->journal2->settings->get('popup_privacy', '1') && $this->journal2->settings->get('popup_privacy_information')) {
				$this->load->model('catalog/information');
				$this->load->language('account/register');

				$information_id = (int)$this->journal2->settings->get('popup_privacy_information');
				$information_info = $this->model_catalog_information->getInformation($information_id);

				if ($information_info) {
					$information = array(
						'information_id' => $information_id,
						'information_title' => $information_info['title'],
						'information_text'	=> sprintf($this->language->get('text_agree'), Journal2Utils::link(version_compare(VERSION, '2', '>=') ? 'information/information/agree' : 'information/information/info', 'information_id=' . $information_id, 'SSL'), $information_info['title'], $information_info['title']),
						'information_error' => htmlspecialchars_decode(sprintf($this->language->get('error_agree'), $information_info['title']))
					);
				} else {
					$information = null;
				}
			...
			$this->journal2->settings->set('popup_privacy_information', $information);

			// gdpr newsletter
			if ($this->journal2->settings->get('newsletter_privacy', '1') && $this->journal2->settings->get('newsletter_privacy_information')) {
				$this->load->model('catalog/information');
				$this->load->language('account/register');

				$information_id = (int)$this->journal2->settings->get('newsletter_privacy_information');
				$information_info = $this->model_catalog_information->getInformation($information_id);
			...
			$this->journal2->settings->set('newsletter_privacy_information', $information);
    ...
    public function columns() {
        ...
        if (($this->journal2->page->getType() === 'product' || $this->journal2->page->getType() === 'quickview')) {
            ...
            if ($this->journal2->settings->get('product_page_options_sold')) {
                $text = $this->journal2->settings->get('product_page_options_sold_text', ' Produk terjual');
