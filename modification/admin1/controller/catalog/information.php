	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
          ...
          if ($this->config->get('marketplace_status') && isset($this->request->get['mpcheck'])) {
              $this->response->redirect($this->url->link('customerpartner/information', 'token=' . $this->session->data['token'] . $url, 'SSL'));
          } else {
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['information_id'])) {
          ...
          if (isset($this->request->get['mpcheck']) && $this->request->get['mpcheck']) {
            $data['action'] = $this->url->link('catalog/information/edit', 'token=' . $this->session->data['token'] . '&mpcheck=1&information_id=' . $this->request->get['information_id'] . $url, 'SSL');
          } else {
