 ...
 // hpwd
		...
		if ($order_info) {
          ...
          if($used_order_statuses && isset($hp_tracking_sort_order[$order_info['order_status_id']])) {
			...
			$nomorvirtual = $this->model_account_order->getNumberVirtual($this->request->get['order_id']);
			$data['nomorvirtual'] = $nomorvirtual['virtual_number'];
			$data['linklacak'] = $nomorvirtual['linklacak'];
			...
			$data['footer'] = $this->load->controller('common/footer');
			$data['header'] = $this->load->controller('common/header');
			$data['cekresi'] = $this->load->controller('common/cekresi');
