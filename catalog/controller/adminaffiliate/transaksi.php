<?php
class ControllerAdminAffiliateTransaksi extends Controller {
    public function index() {
        ...
        $data['heading_title'] = 'Affiliate | Daftar Transaksi';
        ...
        $transaksi = $this->model_affiliate_information->getTransaksi();
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/adminaffiliate/transaksi.tpl')) {
			$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/adminaffiliate/transaksi.tpl', $data));
		} else {
			$this->response->setOutput($this->load->view('default/template/adminaffiliate/transaksi.tpl', $data));
		}
    }
}
?>
