...
class Cart {
	...
	public function __construct($registry) {
		...
		if ($this->customer->getId()) {
			...
			foreach ($cart_query->rows as $cart) {
				$queryX = $this->db->query("SELECT ukuran_unit as ukuran_unit, seller_id as seller_id  FROM " . DB_PREFIX . "cart WHERE cart_id = '" . (int)$cart['cart_id'] . "'");
				$this->db->query("DELETE FROM " . DB_PREFIX . "cart WHERE cart_id = '" . (int)$cart['cart_id'] . "'");

				// The advantage of using $this->add is that it will check if the products already exist and increaser the quantity if necessary.
				$this->add($cart['product_id'], $cart['quantity'], json_decode($cart['option']), $cart['recurring_id'], $queryX->row['ukuran_unit'] , $queryX->row['seller_id']);
			}
		}
	}
	...
	public function getProducts() {
		...
		foreach ($cart_query->rows as $cart) {
            ...
            'total'           => ($price + $option_price + $commission_amount)
           * $cart['quantity'],
...
//	}
	...
	public function add($product_id, $quantity = 1, $option = array(), $recurring_id = 0, $ukuran_unit, $seller_id) {
		//$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "cart WHERE customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "' AND product_id = '" . (int)$product_id . "' AND recurring_id = '" . (int)$recurring_id . "' AND `option` = '" . $this->db->escape(json_encode($option)) . "'");
       $query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "cart WHERE customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "' AND product_id = '" . (int)$product_id . "' AND recurring_id = '" . (int)$recurring_id . "' AND `option` = '" . $this->db->escape(json_encode($option)) . "'");

		if (!$query->row['total']) {
			$this->db->query("INSERT " . DB_PREFIX . "cart SET customer_id = '" . (int)$this->customer->getId() . "', session_id = '" . $this->db->escape($this->session->getId()) . "', product_id = '" . (int)$product_id . "', seller_id = '" . (int)$seller_id . "', ukuran_unit = '" . (int)$ukuran_unit . "', recurring_id = '" . (int)$recurring_id . "', `option` = '" . $this->db->escape(json_encode($option)) . "', quantity = '" . (int)$quantity . "', date_added = NOW()");
           //$this->db->query("INSERT " . DB_PREFIX . "cart SET customer_id = '" . (int)$this->customer->getId() . "', session_id = '" . $this->db->escape($this->session->getId()) . "', product_id = '" . (int)$product_id . "', recurring_id = '" . (int)$recurring_id . "', `option` = '" . $this->db->escape(json_encode($option)) . "', quantity = '" . (int)$quantity . "', date_added = NOW()");
	...
	public function UpdatePaymentMethode($order_id, $payment_methodX) {
     $this->db->query("UPDATE " . DB_PREFIX . "order SET  payment_method = '" . $payment_methodX . "' WHERE order_id = '" . (int)$order_id . "'");
	}
