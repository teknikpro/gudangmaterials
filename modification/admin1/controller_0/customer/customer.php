...
class ControllerCustomerCustomer extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
                ...
                if ($this->config->get('marketplace_status') && isset($this->request->get['create_seller'])) {
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $customer_id) {
				...
				$check_id = $this->model_customer_customer->getMemberMail($customer_id);
										
				$email_member = '';
				if ($check_id) {		
					$email_member = $check_id['email'];
				}		

				$check_id2 = $this->model_customer_customer->getMemberUserID($email_member);
										
				$userid = 0;
				if ($check_id2) {		
					$userid = $check_id2['userid'];
				}

	
				$this->model_customer_customer->deleteMember($email_member,$userid);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['customer_id'])) {
                ...
                if ($this->config->get('marketplace_status') && isset($this->request->get['create_seller'])) {
                    ...
                    $data['action'] = $this->url->link('customer/customer/add', 'token=' . $this->session->data['token'] . $url. '&create_seller', 'SSL');
                }else{
