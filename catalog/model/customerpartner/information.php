...
class ModelCustomerpartnerInformation extends Model {
	public function addInformation($data) {
		...
		if (isset($data['information_description']) && is_array($data['information_description'])) {
		  foreach ($data['information_description'] as $key => $information_description) {
		    if (isset($information_description['description']) && $information_description['description']) {
		      $data['information_description'][$key]['description'] = trim($this->db->escape(htmlentities($this->model_customerpartner_htmlfilter->HTMLFilter(html_entity_decode($information_description['description']),'',true))), '\n');
		    }
		  }
		}
		...
		$this->db->query("INSERT INTO `" . DB_PREFIX . "customerpartner_to_information` SET `information_id` = " . (int)$information_id . ",seller_id = '".(int)$this->customer->getId()."'");
	...
	public function editInformation($information_id, $data) {
		...
		if (isset($data['keyword']) && $data['keyword']) {
		  $data['keyword'] = str_replace("&amp","",$data['keyword']);
$data['keyword'] = preg_replace('/[^A-Za-z0-9_-]/', '', $data['keyword']);
		}
	...
	public function deleteInformation($information_id) {
		...
		$this->db->query("DELETE FROM `" . DB_PREFIX . "customerpartner_to_information` WHERE `information_id` = " . (int)$information_id . " AND seller_id = '".(int)$this->customer->getId()."'");
	...
	public function getInformations($data = array()) {
		if ($data) {
			$sql = "SELECT * FROM " . DB_PREFIX . "information i LEFT JOIN " . DB_PREFIX . "information_description id ON (i.information_id = id.information_id) LEFT JOIN " . DB_PREFIX . "customerpartner_to_information c2i ON (c2i.information_id = i.information_id) WHERE id.language_id = '" . (int)$this->config->get('config_language_id') . "'";

			$sql .= " AND c2i.seller_id = ". (int)$this->customer->getId() ;
	...
	public function getTotalInformations($data) {
		...
		$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "information i LEFT JOIN " . DB_PREFIX . "information_description id ON (i.information_id = id.information_id) LEFT JOIN " . DB_PREFIX . "customerpartner_to_information c2i ON (c2i.information_id = i.information_id) WHERE id.language_id = '" . (int)$this->config->get('config_language_id') . "'";
	...
	public function getSellerInformations($id = 0) {
	  $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "information i LEFT JOIN " . DB_PREFIX . "information_description id ON (i.information_id = id.information_id) LEFT JOIN " . DB_PREFIX . "information_to_store i2s ON (i.information_id = i2s.information_id) LEFT JOIN " . DB_PREFIX . "customerpartner_to_information c2i ON (c2i.information_id = i.information_id) WHERE id.language_id = '" . (int)$this->config->get('config_language_id') . "' AND c2i.seller_id = " . (int)$id . " AND i2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND i.status = '1' ORDER BY i.sort_order, LCASE(id.title) ASC");
