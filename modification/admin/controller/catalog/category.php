...
class ControllerCatalogCategory extends Controller {
	...
	public function add() {
        ...
        if ($this->config->get('marketplace_status') && isset($this->request->get['mpcheck'])) {
            $this->response->redirect($this->url->link('customerpartner/sellercategory', 'token=' . $this->session->data['token'] . $url, 'SSL'));
        } else {
	...
	protected function getForm() {
        ...
        if (isset($this->request->get['mpcheck']) && $this->request->get['mpcheck']) {
          $data['action'] = $this->url->link('catalog/category/edit', 'token=' . $this->session->data['token'] . '&mpcheck=1&category_id=' . $this->request->get['category_id'] . $url, 'SSL');
        } else {
