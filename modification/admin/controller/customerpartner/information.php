...
class ControllerCustomerpartnerInformation extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('customerpartner/information', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		foreach ($results as $result) {
			$data['informations'][] = array(
				...
				'edit'        => $this->url->link('catalog/information/edit', 'token=' . $this->session->data['token'] . '&information_id=' . $result['information_id'] . '&mpcheck=1' . $url, 'SSL'),
				'delete'      => $this->url->link('customerpartner/information/delete', 'token=' . $this->session->data['token'] . '&information_id=' . $result['information_id'] . $url, 'SSL')
			);
		}
		...
		$data['sort_name'] = $this->url->link('customerpartner/information', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$data['sort_sort_order'] = $this->url->link('customerpartner/information', 'token=' . $this->session->data['token'] . '&sort=sort_order' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('customerpartner/information', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('customerpartner/information_list.tpl', $data));
	}
