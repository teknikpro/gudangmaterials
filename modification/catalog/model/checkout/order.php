...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		...
		// get ongkir
		$metode_pengiriman = $this->db->escape($data['shipping_method']);
		$ongkir_query = $this->db->query("SELECT  * FROM oc_order_total WHERE order_id='$order_id' AND code= 'shipping' AND title='$metode_pengiriman' ");
		$ongkir = $ongkir_query->row['value'];

		// updata data
		$this->db->query("UPDATE " . DB_PREFIX . "order_product SET ongkir = '" . $ongkir . "' WHERE order_id = '" . (int)$order_id . "'");
		// $this->db->query("UPDATE " . DB_PREFIX . "order SET comment = '" . $ongkir . "' WHERE order_id = '" . (int)$order_id . "'");

		// add seller to table order
		$seller_query = $this->db->query("SELECT  * FROM oc_order_product WHERE order_product_id ='$order_product_id' ");
		$getproductid = $seller_query->row['product_id'];

		$order_query = $this->db->query("SELECT  * FROM oc_customerpartner_to_product WHERE product_id ='$getproductid' ");
		$getsellerid = $order_query->row['customer_id'];

		$this->db->query("UPDATE " . DB_PREFIX . "order SET ongkir = '" . $ongkir . "', seller_id = '" . $getsellerid . "', status_pembayaran = 1  WHERE order_id = '" . (int)$order_id . "'");
	...
	public function tesOrderData($order_id, $order_status_id, $comment = '', $notify = false, $override = false){
		$event_data = array(
			'order_id'		  => $order_id,
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $virtualnumber = '',  $notify = false, $override = false) {
		$event_data = array(
			'order_id'		  => $order_id,
			'order_status_id' => $order_status_id,
			'comment'		  => $comment,
        ...
        if ($this->config->get('marketplace_status')){
          ...
          $this->model_account_notification->productStockActivity($order_product['product_id']);
			...
			$this->db->query("UPDATE `" . DB_PREFIX . "order` SET order_status_id = '" . (int)$order_status_id . "', date_modified = NOW(), virtual_number = '" . $virtualnumber . "'  WHERE order_id = '" . (int)$order_id . "'");
        ...
        if($this->config->get('marketplace_status') && isset($toAdmin) && $toAdmin) {
