...
class ControllerCustomerpartnerOrder extends Controller {
	...
	public function info() {
		...
		if ($order_info) {
			...
			$data['actionlink'] = $this->url->link('customerpartner/order/linklacak', 'token=' . $this->session->data['token'] . $url, 'SSL');
			...
			$linklacak = $this->model_customerpartner_order->getLinkLacak($order_id);
			$data['linklacak'] = $linklacak['linklacak'];
	...
	Public function linklacak(){
		$this->load->model('customerpartner/konfirmasiongkir');

		$this->session->data['success'] = "Link Lacak Berhasil dibuat";
		$url = '';


		if(isset($this->request->post['linklacak'])){
			$data['linklacak'] = $this->request->post['linklacak'];
		}

		$order_id = $this->request->post['order_id'];
		$seller_id = $this->request->post['seller_id'];

		$this->model_customerpartner_konfirmasiongkir->editLinkLacak($this->request->post);

		$this->response->redirect($this->url->link('customerpartner/order/info', 'token=' . $this->session->data['token'] . '&seller_id=' . $seller_id . '&order_id=' . $order_id . $url, 'SSL'));
