...
class ModelAccountCustomer extends Model {
	...
	public function addLoginAttempt($email) {
		...
		if (!$query->num_rows) {
			...
			//$this->db->query("UPDATE " . DB_PREFIK . "sessionlink SET ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "' WHERE email = '" . $this->db->escape(utf8_strtolower((string)$email)) . "'");
		}
	}
	...
	public function getSessionLink($linkweb,$user_email,$user_emailseller,$from_useridX,$to_useridX,$chat_idX,$from_useridX_m,$to_useridX_m,$linktarget,$ipX){
 	  ...
 	  $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "sessionlink WHERE session_id = '" . $this->db->escape($this->session->getId()) . "'");
			...
			if ($queryX->num_rows) {
				...
				if ($queryY->num_rows) {
				 ...
				 return true;
			  	  ...
			  	  $this->db->query("UPDATE " . DB_PREFIX . "sessionlink SET  ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "' , email = '" . $this->db->escape(utf8_strtolower((string)$user_email)) . "', linktarget = '" . $linktarget . "', from_userid_m = '" . (int)$from_useridX_m . "', to_userid_m = '" . (int)$to_useridX_m . "', from_userid = '" . (int)$from_useridX . "', to_userid = '" . (int)$to_useridX . "', chat_id = '" . (int)$chat_idX . "', link = '" . $linkweb . "', emailseller = '" . $user_emailseller . "' WHERE  session_id = '" . $this->db->escape($this->session->getId()) . "'");
				  return true;
			    }
            }				


     		//$this->db->query("DELETE FROM " . DB_PREFIX . "sessionlink where email like '" . $this->db->escape(utf8_strtolower((string)$user_email)) . "' and session_id != '" . $this->db->escape($this->session->getId()) . "'");
			//$this->db->query("DELETE FROM " . DB_PREFIX . "sessionlink where email=''");
			//$this->db->query("DELETE FROM " . DB_PREFIX . "sessionlink where ip = '" . $ipX . "' and email != '" . $this->db->escape(utf8_strtolower((string)$user_email)) . "'");
			//$this->db->query("UPDATE " . DB_PREFIX . "sessionlink SET  ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "' , email = '" . $this->db->escape(utf8_strtolower((string)$user_email)) . "', linktarget = '" . $linktarget . "', from_userid_m = '" . (int)$from_useridX_m . "', to_userid_m = '" . (int)$to_useridX_m . "', from_userid = '" . (int)$from_useridX . "', to_userid = '" . (int)$to_useridX . "', chat_id = '" . (int)$chat_idX . "', link = '" . $linkweb . "', emailseller = '" . $user_emailseller . "', session_id = '" . $this->db->escape($this->session->getId()) . "'  WHERE email = '" . $this->db->escape(utf8_strtolower((string)$user_email)) . "'");
           


		} else {
			//$this->db->query("DELETE FROM " . DB_PREFIX . "sessionlink where email=''");
			//$this->db->query("DELETE FROM " . DB_PREFIX . "sessionlink where session_id != '" . $this->db->escape($this->session->getId()) . "'");
            $this->db->query("DELETE FROM " . DB_PREFIX . "sessionlink where email like '" . $this->db->escape(utf8_strtolower((string)$user_email)) . "' and session_id != '" . $this->db->escape($this->session->getId()) . "'");
			//$this->db->query("DELETE FROM " . DB_PREFIX . "sessionlink where email != '" . $this->db->escape(utf8_strtolower((string)$user_email)) . "' and session_id = '" . $this->db->escape($this->session->getId()) . "'");
			$this->db->query("INSERT " . DB_PREFIX . "sessionlink SET ip = '" . $ipX . "', linktarget = '" . $linktarget . "', from_userid_m = '" . (int)$from_useridX_m . "', to_userid_m = '" . (int)$to_useridX_m . "', from_userid = '" . (int)$from_useridX . "', to_userid = '" . (int)$to_useridX . "', chat_id = '" . (int)$chat_idX . "', session_id = '" . $this->db->escape($this->session->getId()) . "', link = '" . $linkweb . "', email = '" . $this->db->escape(utf8_strtolower((string)$user_email)) . "', emailseller = '" . $user_emailseller . "'");
             return true;
	...
	public function createMessage($from_useridX_m,$to_useridX_m,$chat_idX){
		...
		$this->db->query("INSERT tbl_chat_message SET iskirim =1, pesan = 'Hallo, ada yg bisa kami bantu', create_date = NOW(), update_date = NOW(), chat_id = '" . (int)$chat_idX . "',  from_userid = '" . (int)$from_useridX_m . "', to_userid = '" . (int)$to_useridX_m . "'");	
