...
class ModelAccountCustomerGroup extends Model {
	...
	public function getCodeAktivasi($codeaktivasi){

		$escaped_code_aktivasi = $this->db->escape($codeaktivasi);
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "customer` WHERE `code_aktivasi` = '" . $escaped_code_aktivasi . "'");

		return $query->row;
	}

	public function updateDataStatus($customer_id){
