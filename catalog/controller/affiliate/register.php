...
class ControllerAffiliateRegister extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->model_affiliate_affiliate->addNotificationAdminRegister();
		...
		if (isset($this->error['provinsi'])) {
			$data['error_provinsi'] = $this->error['rovinsi'];
		} else {
			$data['error_provinsi'] = '';
		}
	...
	protected function validate() {
		...
		if ((utf8_strlen(trim($this->request->post['provinsi'])) < 2) || (utf8_strlen(trim($this->request->post['provinsi'])) > 128)) {
			$this->error['provinsi'] = $this->language->get('error_city');
		}
